%% Success Rate of recovering the nonlinear oscillator System from data
...
usesine = 0;
% Set the parameters of the nonlinear oscillatory system
param.alpha = -0.1;  
param.beta  = -2.0;
param.gamma =  2.0;
param.delta = -0.1;
...
eps_x = 0.01; %(Change according to paper)
...
N_array=21:20:401;
%N_array=21:2:401; (Used in paper)
...
parfor ii=1:length(N_array)
    ...
    [~,x_clean]=ode89(@(t,x) nonlinearoscillator(t,x,param),tspan,x0);
    ...
    for jj=1:TestNum
        ...
        lambda = 0.06; % (Change according to eps_x)
...
% clear par;
% save(['NonlinearOsc_SuccessRate_eps' num2str(eps_x) '_tf' num2str(t_final) '.mat']);
% return
...
if savefig
    saveas(f,[figpath 'NonlinearOsc_SuccessRate_eps' num2str(eps_x) '_tf' num2str(t_final) '.fig']);
    saveas(f,[figpath 'NonlinearOsc_SuccessRate_eps' num2str(eps_x) '_tf' num2str(t_final) '.svg'],'svg');
end
clear f;
